...
## Scope & Principles
- **Default behaviour remains unchanged**: running the LiDAR pipeline with no AOI input must continue to produce the baseline 776 ± 5 detections on the golden tile.
...
1. **Baseline run (no AOI)**
   ...
   make test-lidar
   ```
   - Confirms 776 detections and captures unfiltered outputs for provenance.
...
4. **Regression testing**
   - Extend `tests/test_golden_aoi.py` with a case that runs the script with an AOI covering the golden tile and asserts the count still lands at 776 ± 5.
